...
export interface RemoveSavedShowcaseModalProps {
...
export function RemoveSavedShowcaseModal({
  showcase,
  open,
  onCancel,
  onConfirm,
}: RemoveSavedShowcaseModalProps) {
  if (!open || !showcase) return null;

  const title = showcase.title?.trim() || "Untitled showcase";
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="remove-saved-showcase-title"
    >
      <button
        type="button"
      ...
      <div className="relative w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl dark:border dark:border-white/10 dark:bg-darkBlue-003">
        <div className="border-b border-gray-100 px-5 pb-4 pt-5 dark:border-white/10">
          <div className="flex items-center gap-3">
            ...
            <h2 id="remove-saved-showcase-title" className="text-lg font-bold leading-snug text-gray-900 dark:text-white">
              Remove from Saved
            </h2>
          </div>
          <p className="mt-3 text-sm leading-relaxed text-gray-900 dark:text-gray-100">Remove showcase confirmation</p>
        </div>
        ...
        <div className="px-5 py-4">
          <div className="flex items-center gap-3 rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 dark:border-white/10 dark:bg-darkBlue-203/80">
            <Images className="h-5 w-5 shrink-0 text-gray-800 dark:text-gray-200" aria-hidden />
