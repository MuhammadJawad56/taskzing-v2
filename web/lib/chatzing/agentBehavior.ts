...
export function buildAgentBehaviorPrompt(
  ...
  if (locale === "fr") {
    return [
      ...
      "2) AFFICHE: Si l'utilisateur demande une affiche/poster (ex. « affiche de chien »), génère immédiatement une image avec titre et sous-titre tirés de sa demande. Ne réponds jamais par une liste de capacités ni par des clés API manquantes.",
  ...
  return [
    ...
    "2) POSTER: When the user asks for a poster/flyer (e.g. \"poster of a dog\"), generate an image immediately using title and subtitle from their request. Never reply with a capability list or missing API key messages.",
